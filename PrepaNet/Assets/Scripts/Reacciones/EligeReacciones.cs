...
public class EligeReacciones : MonoBehaviour {
	...
	void IniciaJuego(){
		...
		while (BancoPreguntas.eligeBienReacciones [0, pregunta, 2] == "si") {
			pregunta = (int)Random.Range (0.0f, 28.0f);
		}
		BancoPreguntas.eligeBienReacciones [0, pregunta, 2] = "si";
		//Escribir la pregunta en el título
		arrResp[0].GetComponent<Text>().text = BancoPreguntas.eligeBienReacciones[0,pregunta, 0];
		resp [slotBuena, 0] = BancoPreguntas.eligeBienReacciones [0,pregunta,1];
		resp [slotBuena, 1] = "si";
